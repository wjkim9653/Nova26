...
이 디렉터리의 환자 데이터는 **Synthea™** 로 생성된 **합성(가상) 데이터**입니다. 실제 환자가 아니며
개인식별정보(PII)가 아닙니다.
...
무제한 사용이 가능하며, 출처(아래 인용) 표기만 요구됩니다. 대회 규칙(연구 출판 가능 라이선스 +
출처 명시)을 충족합니다.
...
| `observations.csv` | 관측 기록 원본 전체(시계열). 체온 = LOINC **`8310-5`**, `CATEGORY=vital-signs`, **`VALUE` 는 섭씨(`UNITS=Cel`)**. 환자당 여러 시점 기록 존재. |
| `patient_id_map.csv` | 자체 생성 매핑: `patient_id`(P001…) ↔ `uuid`. 도구가 친숙한 외래키처럼 참조. 같은 디렉터리의 `build_patient_map.py` 로 생성. |
| `build_patient_map.py` | `patients.csv` → `patient_id_map.csv` 생성 스크립트(데이터셋 prep). |
| `dataset.py` | 데이터 로드·조회 모듈(최신 체온 조회 등). `src/` 구현체가 import. |

## 참고

- 체온 원본 단위는 **섭씨**입니다. `get_patient_temperature` 는 미션의 도구 계약에 따라 이를 **화씨로 변환**해 반환합니다.
- 한 환자에 체온 기록이 여러 시점 쌓여 있어, 도구는 **`DATE` 기준 최신 1건**을 선택합니다.
